...
const MapMarkers: React.FC<MapMarkersProps> = ({
  ...
  const handlePreviousConcert = useCallback(async () => {
    ...
    const currentIndex = concerts.findIndex(
      (c) => c.date === infoWindowConcert.date && c.venue === infoWindowConcert.venue,
    );
  ...
  return (
    <>
      {concerts.map((concert, index) => {
        ...
        return (
          <AdvancedMarker
            aria-label={`Concert marker for ${concert.venue} in ${concert.city}, ${
              concert.country
          ...
          >
            <div
              style={{
                ...
                transition: isPulsing ? 'all 0.15s ease-in-out' : 'all 0.1s ease-in-out',
                cursor: 'pointer',
                position: 'relative',
      ...
      {infoWindowConcert && (
        <InfoWindow
          aria-label={`Concert information for ${
            infoWindowConcert.venue
          } in ${infoWindowConcert.city}`}
          maxWidth={450}
          position={{lat: infoWindowConcert.lat, lng: infoWindowConcert.lng}}
          onCloseClick={handleInfoWindowClose}
        >
          <Box sx={{p: 0, minWidth: 350, maxWidth: 450}}>
            {/* Content */}
            <Box sx={{p: 2, pt: 1.5}}>
              {/* Header */}
              <Box sx={{mb: 2}}>
              ...
              >
                ...
                >
                  <Clock aria-hidden="true" color="#f57c00" size={16} />
                  <Typography
                    sx={{
                      color: 'text.primary',
                      fontWeight: 600,
                ...
                {isLoadingContext && (
                  <Box sx={{display: 'flex', alignItems: 'center', gap: 1}}>
                    <CircularProgress aria-label="Loading historical context" size={16} />
                    <Typography
                      sx={{
              ...
              >
                <MusicNote aria-hidden="true" color="#f57c00" size={16} />
                <Typography
                  sx={{
                    fontWeight: 500,
                    fontSize: '0.875rem',
              ...
              <Box
                sx={{display: 'flex', alignItems: 'center', gap: 1, mb: 1.5}}
              >
                <CalendarToday aria-hidden="true" color="#1976d2" size={16} />
                <IconButton
                  aria-label={
                    canGoPrevious ? 'Go to previous concert' : 'No previous concert available'
                  }
                ...
                <Typography
                  ...
                  variant="body2"
                >
                  {new Date(infoWindowConcert.date).toLocaleDateString('en-US', {
                    weekday: 'long',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                ...
                <IconButton
                  aria-label={canGoNext ? 'Go to next concert' : 'No next concert available'}
                  disabled={!canGoNext}
                  size="small"
                  sx={{
              ...
              {infoWindowConcert.leg && (
                <Box
                  sx={{display: 'flex', alignItems: 'center', gap: 1, mb: 1.5}}
                >
                  <MapPin aria-hidden="true" color="#4caf50" size={16} />
                  <Typography
                    sx={{
                      fontWeight: 500,
                      fontSize: '0.875rem',
              ...
              {infoWindowConcert.event && (
                <Box sx={{display: 'flex', alignItems: 'center', gap: 1}}>
                  <MusicNote aria-hidden="true" color="#9c27b0" size={16} />
                  <Typography
                    sx={{fontSize: '0.875rem', color: 'text.secondary'}}
                    variant="body2"
                  >
